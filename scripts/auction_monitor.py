...
def atomic(path,value):
 ...
 os.replace(tmp,path)
def load_plan(day):return json.loads((BASE/'outputs'/'plans'/f'{day}_plan.json').read_text(encoding='utf-8'))
def observe(now):
 day=now.strftime('%Y-%m-%d');hm=now.strftime('%H:%M');plan=load_plan(day);ledger=load();syms=load_universe();codes=[tencent_symbol(s) for s in syms];quotes={};fail=[]
 ...
 result={'date':day,'time':now.strftime('%H:%M:%S'),'phase':'cancelable' if hm<'09:20' else('no_cancel' if hm<'09:25' else'final'),'coverage':round(coverage,4),'universe_count':len(syms),'quote_count':len(quotes),'focus':focus,'active_top':active,'read_only':True}
 ...
 print(json.dumps({'phase':result['phase'],'coverage':result['coverage'],'focus':len(focus),'active':len(active),'frozen':hm>='09:25'},ensure_ascii=False));return 0
