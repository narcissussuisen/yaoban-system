...
def main():
    ...
    ap.add_argument('--gate-mode', default='leak', help='leak=提交版行为(w泄漏变量); own=按当前候选自身属性; none=关闭')
    ap.add_argument('--gate-log', default='', help='gate日志CSV路径')
    ...
    for di, d in enumerate(calendar):
        ...
        if args.gate_log and watch:
            gate_log.append({'date': d, 'sym': '__WATCH__', 'own_yang': str([w[2] for w in watch]),
                             'own_brk5': str([w[3] for w in watch]),
                             'leak_sym': w[0] if watch else '', 'leak_sd': w[1] if watch else '',
                             'leak_yang': w[2] if watch else '', 'leak_brk5': w[3] if watch else '',
                             'own_pass': '', 'leak_pass': '', 'decision': 'watch_top4', 'confirm': ''})
        # ---- 盘中: 三引擎确认买入
        bought_today = []
        n_pos = len(pos)
        for (sym, sd, yang, brk5, buy_yang, buy_vol_up) in watch:
            ...
            if args.require_gate:
                # 信号日 yang & brk5 硬条件
                own_pass = (yang == 'True' and brk5 == 'True')
                leak_pass = (w[2] == 'True' and w[3] == 'True')
                if args.gate_mode == 'own':
                    gate_pass = own_pass
                elif args.gate_mode == 'none':
                    gate_pass = True
                else:  # leak = committed behavior
                    gate_pass = leak_pass
                if args.gate_log:
                    gate_log.append({'date': d, 'sym': sym, 'own_yang': yang, 'own_brk5': brk5,
                                     'leak_sym': w[0], 'leak_sd': w[1], 'leak_yang': w[2], 'leak_brk5': w[3],
                                     'own_pass': own_pass, 'leak_pass': leak_pass,
                                     'decision': 'buy' if gate_pass else 'blocked',
                                     'confirm': str(cf is not None)})
                if not gate_pass:
                    continue
            px, kind, t = cf
            if args.verbose:
    ...
    if args.gate_log:
        pd.DataFrame(gate_log).to_csv(args.gate_log, index=False)
    print()
