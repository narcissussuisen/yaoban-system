"""R6' REVIEW PROBE (no-lookahead: k=1..5 window + ranking w/o future attrs) — derived from 922ab78 v1：候选表 + 三引擎盘中确认 + 卖出引擎 + 组合级联动 + 温度计仓位档
...
def main():
    ...
    for di, d in enumerate(calendar):
        ...
        if True:
            ...
            watch.sort(key=lambda w: (w[3] == 'True', w[2] == 'True'), reverse=True)  # REVIEW: no future attrs
    ...
    eq_df.to_csv(BASE / 'outputs' / '_review_r6p_equity_probe_nolookahead.csv', index=False)
    fills_df = pd.DataFrame(fills)
    fills_df.to_csv(BASE / 'outputs' / '_review_r6p_fills_probe.csv', index=False)
    buys = fills_df[fills_df['side'] == 'buy']
    print(f'\n[PROBE] total fills={len(fills_df)} buys={len(buys)} sells={len(fills_df)-len(buys)}')
    if 'cand_k' in buys.columns:
        print('[PROBE] buy cand_k distribution:', buys['cand_k'].value_counts().sort_index().to_dict())
    sells = fills_df[fills_df['side'] == 'sell']
    print('[PROBE] sell reason distribution:')
    print(sells['kind'].value_counts().to_string())
    print()
    print('资金曲线(probe): outputs/_review_r6p_equity_probe_nolookahead.csv')
