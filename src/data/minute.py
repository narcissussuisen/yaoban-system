...
# 2026-09-10 根因更正: 旧清单(115.238.56.198 等)实测 TCP 通但 bars 返空 —— 是 mootdx "读取不了" 的真因,
# 非服务端故障。下表 117.34.114.* 段实测 mootdx+pytdx 双库取数验活通过(60m 800 根, 与账本收盘吻合)。
PYTDX_SERVERS = [
    ("117.34.114.13", 7709),  # 主：2026-09-10 实测验活
    ("117.34.114.14", 7709), ("117.34.114.15", 7709), ("117.34.114.16", 7709),
    ("117.34.114.17", 7709), ("117.34.114.18", 7709), ("117.34.114.20", 7709),
    ("115.238.56.198", 7709),  # 旧节点保留备用(当前返空)
    ("123.125.108.14", 7709),
]
